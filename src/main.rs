...
//   2       2    Channel ID
//   4       4    Packet Length (total, including this header)
//   8       4    Data Length
//  12       1    Header Version
//  13       1    Sequence Number
//  14       1    Packet Flags
//  15       1    Data Type
//  16       6    Relative Time Counter (48-bit)
//  22       2    Header Checksum
//
// Secondary header (optional, indicated by bit 2 of flags) adds bytes after
// the primary header. We skip into data by packet_length, so we don't need
// to parse it for a structural listing.
...
struct PacketHeader {
    ...
    packet_flags: u8,
    data_type: u8,
    rtc: u64,              // 48-bit relative time counter stored in u64
    checksum_stored: u16,  // the checksum value read from the header
    checksum_valid: bool,  // did it match our computed value?
}

/// Compute the IRIG 106 header checksum.
///
/// The checksum is the arithmetic sum of the first 11 little-endian
/// 16-bit words (bytes 0–21), truncated to 16 bits. The result is
/// stored in bytes 22–23. This is the "calibration value" that
/// confirms a sync pattern is a real packet header and not coincidental
/// payload data.
fn compute_header_checksum(buf: &[u8]) -> u16 {
    let mut sum: u32 = 0;
    for i in 0..11 {
...
/// Check if the 24-byte buffer at this position has both a valid sync
/// word AND a matching header checksum. This is the robust way to
/// distinguish real headers from coincidental 0xEB25 in payload data.
fn is_valid_header(buf: &[u8]) -> bool {
...
impl PacketHeader {
    /// Parse a 24-byte header from a byte slice. Returns None if too short
    /// or sync word doesn't match. Validates the header checksum and reports
    /// the result in `checksum_valid`.
    ...
    #[allow(dead_code)]
    fn checksum_type(&self) -> &'static str {
    ...
    fn data_overflow(&self) -> bool {
        (self.packet_flags & 0x40) != 0
    }

    fn rtc_sync_error(&self) -> bool {
...
impl ChannelStats {
    ...
    fn update(&mut self, hdr: &PacketHeader, packet_num: u64, errors: &mut Vec<String>) {
        self.packet_count += 1;
        ...
        // L3-ROB-020/021: Sequence number gap detection
...
fn main() {
    ...
    // Memory-map the file for maximum throughput
    ...
    if file_size < HEADER_SIZE as u64 {
        eprintln!("Error: File too small to contain a CH.10 packet ({} bytes, minimum is {})",
    ...
    const MAX_RECOVERY_SCAN: usize = 1024 * 1024; // L3-ROB-011: 1 MiB
    ...
    // L3-ROB-030: Track what the first packet is for TMATS position validation
    let mut first_packet_channel: Option<u16> = None;
    let mut first_packet_type: Option<u8> = None;
    let mut tmats_packet_num: Option<u64> = None; // L3-ROB-031
    ...
    loop {
        ...
        match PacketHeader::parse(&mmap[offset..]) {
            Some(hdr) => {
                // L3-ROB-002: Zero-length packet (infinite loop prevention)
                if hdr.packet_length == 0 {
                    errors.push(format!(
                        "Packet {} @ offset {:#X}: packet_length is ZERO (infinite loop avoided), entering recovery",
                        packet_num, offset
                    ));
                    offset += 1; // advance by 1 to avoid re-reading same header
                    continue;    // will re-enter loop and likely hit recovery
                }

                // L3-ROB-001: packet_length below minimum header size
                ...
                // L3-ROB-003: Truncation detection
                ...
                if next_offset > mmap.len() {
                    ...
                    // Still count this packet's header info before stopping
                ...
                // L3-ROB-004: data_length vs packet_length consistency
                ...
                // L3-ROB-030/031: Track first packet and TMATS position
                if packet_num == 0 {
                ...
                if verbose {
                    let chk_mark = if hdr.checksum_valid { " ✓" } else { " ✗" };
                    println!(
            ...
            None => {
                // Try to find next VALID header (sync word + checksum match)
                ...
                for i in search_start..search_end.saturating_sub(HEADER_SIZE) {
                    // L3-ROB-012: Short-circuit — check sync first, then checksum
    ...
    // Count total sequence gaps across all channels
    ...
    println!("│  SUMMARY                                                     │");
    println!("├──────────────────────────────────────────────────────────────┤");
    println!("│  Total packets: {:<44}│", packet_num);
    println!("│  Channels found: {:<43}│", channels.len());
    println!(
        "│  Bytes parsed: {:<45}│",
        format!("{} of {}", HumanBytes(offset as u64), HumanBytes(file_size))
    );
    if total_checksum_failures == 0 {
        println!("│  Header checksums: ✓ ALL PASSED{:<29}│", "");
    } else {
        println!(
            "│  Header checksums: ✗ {} FAILED{:<29}│",
            total_checksum_failures, ""
        );
    }
    if total_sequence_gaps == 0 {
        println!("│  Sequence numbers: ✓ NO GAPS{:<32}│", "");
    } else {
        println!(
            "│  Sequence numbers: ✗ {} GAPS detected{:<24}│",
            total_sequence_gaps, ""
        );
    }
    println!("└──────────────────────────────────────────────────────────────┘");
    ...
    for (&ch_id, stats) in &channels {
        ...
        if stats.overflow_count > 0 || stats.sync_error_count > 0 || stats.checksum_failures > 0 || stats.sequence_gaps > 0 {
    ...
    let has_tmats = channels.get(&0).map_or(false, |s| s.data_type == 0x00);
    ...
    if has_tmats {
        ...
        match tmats_packet_num {
            ...
            Some(n) => {
                println!("    ⚠ TMATS found at packet #{} instead of packet #0.", n);
            ...
            None => {} // shouldn't happen if has_tmats is true
        }
    } else {
        ...
        println!("      - EU calibration coefficients (raw→engineering unit conversion polynomials)");
        ...
        if let (Some(ch), Some(dt)) = (first_packet_channel, first_packet_type) {
            println!("    First packet was: Channel {}, Type {:#04X} ({})",
                ch, dt, data_type_name(dt));
        }
    }
    println!();
}
...
mod tests {
    ...
    fn rejects_header_with_bad_checksum_even_when_sync_matches() {
        ...
        assert_eq!(parsed.checksum_stored, u16::from_le_bytes([header[22], header[23]]));
    }

    #[test]
    fn parses_packet_header_fields_and_flags() {
        let header = make_header(42, 128, 96, 9, 0xC5, 0x40, [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF]);
