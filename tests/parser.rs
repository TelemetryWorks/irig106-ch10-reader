...
#[test]
fn cli_reports_indexing_not_found_when_no_index_packets_exist() {
    let file_bytes = make_header(7, HEADER_SIZE as u32, 0, 0, 0x00, 0x11, [0; 6]).to_vec();

    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("valid system time")
        .as_nanos();
    let path = std::env::temp_dir().join(format!("ch10r-no-index-{unique}.ch10"));
    fs::write(&path, file_bytes).expect("write test ch10 file");
